...
为什么要分不同加载器？   
* 加载器的作用大致就是读取编译后规范的java类数据源（可以是文件系统上的.java文件、远程数据等），通过本地方法 `Class defineClass(String name, byte[] b, int off, int len)`加载到JVM方法区，之后可以再生成对应Object类
* 所以为了不同的数据源就需要不同的加载器
* 另外如果需要再一个JVM上运行多个程序，那么就要避免类同名、版本冲突，如tomcat容器
...
[深入理解jvm-虚拟机类加载机制-参考《深入理解Java虚拟机——JVM高级特性与最佳实践》](https://crowhawk.github.io/2017/08/21/jvm_5/)
